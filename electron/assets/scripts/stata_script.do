...
filefilter $fileDir{3}temp.txt $fileDir{3}tempA.txt, from ("\038d") to ("&amp;") replace
filefilter $fileDir{3}tempA.txt $fileDir{3}tempB.txt, from ("\060d") to ("<") replace
filefilter $fileDir{3}tempB.txt $fileDir{3}tempC.txt, from ("\062d") to (">") replace
filefilter $fileDir{3}tempC.txt $fileDir{3}tempD.txt, from ("\039d") to ("&apos;") replace
filefilter $fileDir{3}tempD.txt $fileDir{3}tempE.txt, from ("\034d") to ("&quot;") replace
end
...
cls
describe _all
translate @Results $fileDir{3}temp.txt, replace
clean
...
import delimited v1 using $fileDir{3}tempE.txt, clear ///
...
rename var4 var3
gen v1=_n
save $fileDir{3}valLabelName, replace
* Make new dataset
use $inputStata, replace
cls
format _all
...
import delimited v1 v2 using $fileDir{3}tempE.txt, clear ///
delimiter("%") stringcols(_all) encoding("utf-8")
replace v1=trim(v1)
drop if v2==""
replace v2="%"+trim(v2)
...
gen v1=_n
order v1 var1 var2
save $fileDir{3}varNameFormat, replace
* Create output with variable name, format and codelist ref.
merge 1:1 v1 using $fileDir{3}valLabelName, nogenerate
drop v1
replace var3=var3+"." if var3!=""
gen var=var1+" "+var2+" "+var3
drop var1 var2 var3
* Write output to file
export delimited using $fileDir{3}tempB.txt, delimiter(",") novarnames nolabel replace
filefilter $fileDir{3}tempB.txt $outDir{3}{2}_VARIABEL.txt, from(",$osEOL") to ("") replace
...
merge 1:1 v1 using $fileDir{3}varNameFormat, nogenerate
* If label is empty, use default value
replace varLabel="n.a." if varLabel==""
gen v0=var1+" '"+varLabel+"'"
keep v0
* Write output to file
export delimited using $outDir{3}{2}_VARIABELBESKRIVELSE.txt, delimiter(tab) novarnames nolabel replace
...
keep valLabel
* Write output to file
export delimited using $outDir{3}{2}_KODELISTE.txt, delimiter(tab) novarnames nolabel replace
}
else {
cls
...
erase $fileDir{3}temp.txt
erase $fileDir{3}tempA.txt
erase $fileDir{3}tempB.txt
erase $fileDir{3}tempC.txt
erase $fileDir{3}tempD.txt
erase $fileDir{3}tempE.txt
erase $fileDir{3}valLabelName.dta
erase $fileDir{3}varNameFormat.dta
...
export delimited using $outDir{3}$inputStata_.csv, delimiter(";") nolabel replace