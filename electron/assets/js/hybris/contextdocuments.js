    ...
    function (n) {
        ...
        var RenderDocuments = function(data) {
            if(data != null && data.toString() != null && data.toString() !== "") {
                ...
                settings.documents.forEach(upload => {
                    $(settings.uploadsTbl).append("<tr><td>{0}</td><td>{1}</td><td><input type=\"text\" id=\"hybris-contextdocuments-document-{0}\" class=\"path\" text=\"{2}\" readonly=\"true\" placeholder=\"Vælg sti med knappen\"/></td><td><button id=\"hybris-contextdocuments-selectFile-{0}\">Browse</button></td></tr>".format(upload.id,upload.title,upload.path));
